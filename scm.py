...
def GenFakeDiff(filename):
  ...
  file_content = depends_utils.FileRead(filename, 'rb').splitlines(True)
...
class GIT(object):
  ...
  def CaptureStatus(files, cwd, upstream_branch):
    ...
    if upstream_branch is None:
      ...
      if upstream_branch is None:
        raise depends_utils.Error('Cannot determine upstream branch')
  ...
  def GetBlessedSha1ForSvnRev(cwd, rev):
    ...
    try:
      ...
      if git_svn_rev != output.rstrip():
        raise depends_utils.Error(sha1)
      return sha1
    except subprocess2.CalledProcessError:
      return None
