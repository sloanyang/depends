...
# Files
#   .depends      : Current client configuration, written by 'config' command.
...
#                   the DEPS file below.
#   .depends_entries : A cache constructed by 'update' command.  Format is a
...
#
# Hooks
#   .depends and DEPS files may optionally contain a list named "hooks" to
...
#                any.  An action command will run at most one time per depends
#                invocation, regardless of how many files matched the pattern.
#                The action is executed in the same directory as the .depends
...
#   An optional key named "target_os" may be added to a depends file to specify
...
class GClientKeywords(object):
  ...
  class VarImpl(object):
    ...
    def Lookup(self, var_name):
      ...
      raise depends_utils.Error("Var is not defined: %s" % var_name)
...
class DependencySettings(GClientKeywords):
  ...
  def __init__(
    ...
    # depends after depends checks it out initially.  The difference between
    # 'managed' and 'should_process' is that the user specifies 'managed' via
    # the --unmanaged command-line flag or a .depends config, where
    # 'should_process' is dynamically set by depends if it goes over its
    # recursion limit and controls depends's behavior so it does not misbehave.
    ...
    # These are only set in .depends and not in DEPS files.
...
class Dependency(depends_utils.WorkItem, DependencySettings):
  ...
  def __init__(self, parent, name, url, safesync_url, managed, custom_deps,
    ...
    depends_utils.WorkItem.__init__(self, name)
    DependencySettings.__init__(
        self, parent, url, safesync_url, managed, custom_deps, custom_vars,
        custom_hooks, deps_file, should_process)

    # This is in both .depends and DEPS files:
    self._deps_hooks = []
    ...
    if not self.name and self.parent:
      raise depends_utils.Error('Dependency without name')
  ...
  def requirements(self):
    ...
    # thus unsorted, while the .depends format is a list thus sorted.
    #
  ...
  def LateOverride(self, url):
    ...
    if isinstance(url, self.FromImpl):
      ...
      if not ref:
        raise depends_utils.Error('Failed to find one reference to %s. %s' % (
            url.module_name, ref))
    ...
    if isinstance(url, basestring):
      ...
      if not parsed_url[0]:
        ...
        scm = depends_scm.CreateSCM(parent_url, self.root.root_dir, None)
    ...
    raise depends_utils.Error('Unknown url type')
  ...
  def ParseDepsFile(self):
    ...
    if not os.path.isfile(filepath):
      ...
      deps_content = depends_utils.FileRead(filepath)
      logging.debug('ParseDepsFile(%s) read:\n%s' % (self.name, deps_content))
      # Eval the content.
      try:
        exec(deps_content, global_scope, local_scope)
      except SyntaxError, e:
        depends_utils.SyntaxErrorToError(filepath, e)
  ...
  def maybeGetParentRevision(
    ...
    if command == 'update' and options.transitive and not options.revision:
      _, revision = depends_utils.SplitUrlRevision(parsed_url)
      if not revision:
        options.revision = revision_overrides.get(parent_name)
        if (options.revision and
            not depends_utils.IsDateRevision(options.revision)):
          ...
          if self.url.startswith('/'):
            ...
            parent_revision_date = self.parent.used_scm.GetRevisionDate(
                options.revision)
            options.revision = depends_utils.MakeDateRevision(
  ...
  def run(self, revision_overrides, command, args, work_queue, options):
    ...
    if run_scm and parsed_url:
      if isinstance(parsed_url, self.FileImpl):
        ...
        if not command in (None, 'cleanup', 'diff', 'pack', 'status'):
          ...
          self._used_scm = depends_scm.SVNWrapper(
        ...
        self._used_scm = depends_scm.CreateSCM(
    ...
    if command == 'recurse':
      if not isinstance(parsed_url, self.FileImpl):
        ...
        scm = depends_scm.GetScmName(parsed_url)
        if not options.scm or scm in options.scm:
          ...
          if parsed_url is None:
            ...
            try:
              depends_utils.CheckCallAndFilter(
  ...
  def GetHooks(self, options):
    ...
    if self.deps_hooks:
      ...
      if (options.force or
          ...
          depends_scm.GetScmName(self.parsed_url) in ('git', None) or
        ...
        # Run hooks on the basis of whether the files from the depends operation
  ...
  def RunHooksRecursively(self, options):
    ...
    for hook in self.GetHooks(options):
      try:
      ...
      except (depends_utils.Error, subprocess2.CalledProcessError), e:
        ...
        if elapsed_time > 10:
          print "Hook '%s' took %.2f secs" % (
              depends_utils.CommandToStr(hook), elapsed_time)
...
class GClient(Dependency):
  """Object that represent a depends checkout. A tree of Dependency(), one per
...
# Snapshot generated with depends revinfo --snapshot
solutions = [
%(solution_list)s]
""")
  ...
  def SetConfig(self, content):
    ...
    try:
      ...
      depends_utils.SyntaxErrorToError('.depends', e)
    ...
    depends_scm.GitWrapper.cache_dir = config_dict.get('cache_dir')

    if not target_os and config_dict.get('target_os_only', False):
      raise depends_utils.Error('Can\'t use target_os_only if target_os is '
                                'not specified')
    ...
    for s in config_dict.get('solutions', []):
      try:
        ...
        raise depends_utils.Error('Invalid .depends file. Solution is '
                                  'incomplete: %s' % s)
    self.add_dependencies_and_close(deps_to_add, config_dict.get('hooks', []))
    logging.info('SetConfig() done')

  def SaveConfig(self):
    depends_utils.FileWrite(os.path.join(self.root_dir,
                                         self._options.config_filename),
                            self.config_content)

  @staticmethod
  def LoadCurrentConfig(options):
    """Searches for and loads a .depends file relative to the current working
    dir. Returns a GClient object."""
    if options.spec:
      client = GClient('.', options)
      client.SetConfig(options.spec)
    else:
      path = depends_utils.FindGclientRoot(os.getcwd(), options.config_filename)
      if not path:
        return None
      client = GClient(path, options)
      client.SetConfig(depends_utils.FileRead(
    ...
    if (options.revisions and
      ...
      print >> sys.stderr, (
          ...
          'when you have multiple solutions setup in your .depends file.\n'
  ...
  def _SaveEntries(self):
    """Creates a .depends_entries file to record the list of unique checkouts.

    The .depends_entries file lives in the same directory as .depends.
    """
    ...
    depends_utils.FileWrite(file_path, result)

  def _ReadEntries(self):
    """Read the .depends_entries file for the given client.
    ...
    if not os.path.exists(filename):
      return {}
    try:
      exec(depends_utils.FileRead(filename), scope)
    except SyntaxError, e:
      depends_utils.SyntaxErrorToError(filename, e)
  ...
  def _EnforceRevisions(self):
    ...
    for revision in self._options.revisions:
      ...
      if not sol in solutions_names:
        #raise depends_utils.Error('%s is not a valid solution.' % sol)
  ...
  def _ApplySafeSyncRev(self, dep):
    ...
    if not rev:
      raise depends_utils.Error(
          ...
          dep.safesync_url)
    scm = depends_scm.CreateSCM(dep.url, dep.root.root_dir, dep.name)
  ...
  def RunOnDeps(self, command, args, ignore_requirements=False, progress=True):
    ...
    if not self.dependencies:
      raise depends_utils.Error('No solution specified')
    revision_overrides = {}
    ...
    work_queue = depends_utils.ExecutionQueue(
    ...
    if command == 'update':
      ...
      for entry, prev_url in self._ReadEntries().iteritems():
        if not prev_url:
          # entry must have been overridden via .depends custom_deps
          continue
        ...
        if (entry not in entries and
          ...
          scm = depends_scm.CreateSCM(prev_url, self.root_dir, entry_fixed)
  ...
  def PrintRevInfo(self):
    ...
    work_queue = depends_utils.ExecutionQueue(self._options.jobs, None, False)
    for s in self.dependencies:
      work_queue.enqueue(s)
    ...
    def GetURLAndRev(dep):
      ...
      url, _ = depends_utils.SplitUrlRevision(original_url)
      scm = depends_scm.CreateSCM(original_url, self.root_dir, dep.name)
    ...
    if self._options.snapshot:
      ...
      for d in self.dependencies:
        ...
        new_depends += self.DEFAULT_SNAPSHOT_SOLUTION_TEXT % {
      ...
      print(self.DEFAULT_SNAPSHOT_FILE_TEXT % {'solution_list': new_depends})
    else:
      entries = {}
  ...
  def ParseDepsFile(self):
    """No DEPS to parse for a .depends file."""
    raise depends_utils.Error('Internal error')

  @property
  def root_dir(self):
    """Root directory of depends checkout."""
...
def CMDcleanup(parser, args):
  ...
  if not client:
    raise depends_utils.Error('client not configured; see \'depends config\'')
  if options.verbose:
    # Print out the .depends file.  This is longer than if we just printed the
...
def CMDrecurse(parser, args):
  ...
  if not args:
    ...
    return 1
  root_and_entries = depends_utils.GetGClientRootAndEntries()
  if not root_and_entries:
    print >> sys.stderr, (
        'You need to run depends sync at least once to use \'recurse\'.\n'
        'This is because .depends_entries needs to exist and be up to date.')
    return 1
...
def CMDgrep(parser, args):
  """Greps through git repos managed by depends.
  ...
  if not args or re.match('(-h|--help)$', args[0]):
    print >> sys.stderr, (
        'Usage: depends grep [-j <N>] git-grep-args...\n\n'
        'Example: "depends grep -j10 -A2 RefCountedBase" runs\n"git grep '
        '-A2 RefCountedBase" on each of depends\'s git\nrepos with up to '
...
def CMDconfig(parser, args):
  """Creates a .depends file in the current directory.
  ...
  URL.
  """
  # We do a little dance with the --dependsfile option.  'depends config' is the
  # only command where it's acceptable to have both '--dependsfile' and '--spec'
  # arguments.  So, we temporarily stash any --dependsfile parameter into
  # options.output_config_file until after the (dependsfile xor spec) error
  # check.
  parser.remove_option('--dependsfile')
  parser.add_option('--dependsfile', dest='output_config_file',
                    help='Specify an alternate .depends file')
  ...
  parser.add_option('--unmanaged', action='store_true', default=False,
                    help='overrides the default behavior to make it possible '
                         'to have the main solution untouched by depends '
                         '(depends will check out unmanaged dependencies but '
...
@subcommand.epilog("""Examples:
  ...
  depends sync --revision src@31000
...
are present in the depends solution, but were not processed (due to custom_deps,
os_deps, etc.)
...
def CMDsync(parser, args):
  ...
  parser.add_option('-r', '--revision', action='append',
                    ...
                    help='Enforces revision/hash for the solutions with the '
                         ...
                         'skipped. -r can be used multiple times when .depends '
...
def CMDrevinfo(parser, args):
  ...
  parser.add_option('-a', '--actual', action='store_true',
                    help='gets the actual checked out revisions instead of the '
                         'ones specified in the DEPS and .depends files')
  parser.add_option('-s', '--snapshot', action='store_true',
                    help='creates a snapshot .depends file of the current '
...
def CMDhookinfo(parser, args):
  """Outputs the hooks that would be run by `depends runhooks`."""
  (options, args) = parser.parse_args(args)
  options.force = True
  client = GClient.LoadCurrentConfig(options)
  if not client:
...
class OptionParser(optparse.OptionParser):
  dependsfile_default = os.environ.get('GCLIENT_FILE', '.depends')
  ...
  def __init__(self, **kwargs):
    ...
    if platform.machine().startswith('arm'):
      ...
      jobs = max(8, depends_utils.NumLocalCpus())
    # cmp: 2013/06/19
    ...
    self.add_option(
        '--dependsfile', dest='config_filename',
        help='Specify an alternate %s file' % self.dependsfile_default)
    self.add_option(
        '--spec',
        help='create a depends file containing the provided string. Due to '
  ...
  def parse_args(self, args=None, values=None):
    ...
    if options.config_filename and options.spec:
      self.error('Cannot specifiy both --dependsfile and --spec')
    if not options.config_filename:
      options.config_filename = self.dependsfile_default
...
def disable_buffering():
  ...
  sys.stdout = depends_utils.MakeFileAutoFlush(sys.stdout)
  # Make stdout annotated with the thread ids.
  sys.stdout = depends_utils.MakeFileAnnotated(sys.stdout)
...
def Main(argv):
  ...
  try:
    ...
    depends_utils.GClientChildren.KillAllRemainingChildren()
    raise
  except (depends_utils.Error, subprocess2.CalledProcessError), e:
    print >> sys.stderr, 'Error: %s' % str(e)
    return 1
